...
const TabRoute = ({ authModel, tabModel }) => {
  ...
  useEffect(() => {
    ...
    const { icon, key, title } = matchs[matchs.length - 1];
    const tabItem = {
      icon,
      key,
      title: title + (location?.state?.pageTitle || ''),
  ...
  return (
    <Tabs activeKey={activeTab.pathname} className='app-tab-list' type='card'>
      {tabList.map((v) => {
              ...
              <Dropdown
                ...
                overlay={
                  <div className='tab-context-menu'>
                    <Button type='link' block className='context-item' disabled={activeTab.pathname !== v.pathname || isErrorPage} onClick={onRefresh}>
                      刷新
                    </Button>
                    <Button type='link' block className='context-item' disabled={isHomePage || !closeAble} onClick={onClose}>
                      关闭
                    </Button>
                    <Button type='link' className='context-item' disabled={!closeAble || isErrorPage} onClick={onCloseOther}>
                      关闭其他
                    </Button>
                    <Button type='link' className='context-item' disabled={isHomePage || !closeAble || isErrorPage} onClick={closeAll}>
                      关闭所有
                    </Button>
                  </div>
                }
              ...
              >
                <Link to={v.location} onContextMenu={(e) => onContextMenu(e, v)}>
                  <div className='app-tab-item'>
                    <span>{v.title}</span>
                    {closeAble && !isHomePage && <CloseOutlined onClick={(e) => onTabClose(e, v)} />}
                  </div>
                </Link>
              </Dropdown>
            }
