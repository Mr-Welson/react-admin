...
class AuthModel extends StoreEnhancer {
  ...
  updateRouteList = (remoteRouteList) => {
    ...
    pageRoutes.forEach((v) => {
      ...
      let { title, icon, ...rest } = v;
      let route = {
        ...rest,
        title: remoteRoute?.meta?.title || title,
        icon: remoteRoute?.meta?.icon || icon,
        routes: undefined,
      };
      if (v.routes && v.routes.length) {
        ...
        v.routes.forEach((sub) => {
          ...
          if (sub.authKey) {
            // 操作权限
            remoteRoute = this.authList.find((x) => x.authKey === sub.authKey);
          } else {
            // 子菜单
            remoteRoute = flatRemoteRoutes.find((x) => x.name === sub.key);
          }
          if (remoteRoute) {
            let { title, icon, ...rest } = sub;
            subRoute = {
              ...rest,
              title: remoteRoute.meta?.title || title,
              icon: remoteRoute.meta?.icon || icon,
